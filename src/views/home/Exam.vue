<template>
  <div class="exam" :key="$route.path">
    ...
    <el-dialog
      ...
      <div class="display-flex ai-center fd-column">
        <p class="fs-16">你此次得分为</p>
        <span class=" mb-20 fs-32 color-primary">{{userScore}}</span>
        <el-button type="success" class="width-percent-80" round @click="$router.replace(`/home/exam-detail?id=${examId}`)">查看得分详情</el-button>
  ...
  methods: {
    ...
    submit () {
      ...
      if (isSubmit) {
        ...
        this.$api.exercise.examSubmit(data).then(res => {
    ...
    // 获取题目
    examStart () {
      this.$api.exercise.examStart({ examId: this.$route.query.id }).then(res => {
    ...
    // 查看得分详情
    getExamInfo () {
      this.$api.exercise.getExamInfo({ id: this.$route.query.id }).then(res => {
